...
```
**Q0 explanation**
Clearly, FreshOrMales has more obs. than FreshMales, since 'and' is more restrictive. 
...
```{r Q1}
KimDataPhysical <- KimDataPhysical  %>%
  mutate(KimDataPhysical, BMI = 703 * Weight/Height^2) %>%
  mutate(KimDataPhysical, Obese = BMI >= 30)
head(KimDataPhysical)
...
```
**Q2 Explanation** `BMI` There is no direct relation between semester here at Truman and BMI. We can see that freshmen have a very high BMI, and also 5th year students. Other than that, the BMI has no obvious change. 
...
KimDataPhysical %>%
  group_by(Year) %>%
  summarize(n = n(), Obese.Pct = 100 * mean(Obese == TRUE, na.rm = TRUE))
```
**Q3 Explanation** There is no linear relation between the results, but what is obvious, is that the number of obese students drops in the later years after coming here. 
...
```{r Q4}
KimBMI <- KimDataPhysical %>%
  group_by(Gender, Year) %>% 
  summarize(averageBMI = mean(BMI, na.rm = TRUE), n = n())
  
KimBMI
```
**Q4 Explanation** Females have a smaller BMI than males. Males seem to keep having the same BMI throughout the years, while females have a  smaller BMI after finishing their first semester here. Groups where statistics are most suspect are the last three years. 
...
```{r Q5}
ggplot(KimDataPhysical) + 
  geom_histogram(mapping = aes(x = `Shoe Size`),
                 fill = 'tan',
                 color = 'black',
                 binwidth = 1.5)
```
**Q5 Explanation** We see that we have an outlier, pointing to a shoe size of over 100, which makes no sense. 
...
KimDataPhysical %>%
  group_by(Gender) %>%
  summarize(averageSize = mean(`Shoe Size`, na.rm = TRUE))
  #End of the first command
KimDataPhysical %>%
  group_by(Gender) %>%
  filter(`Shoe Size` < 20) %>%
  summarize(averageSizeNoOutlier = mean(`Shoe Size`, na.rm = TRUE))


```
**Q6 Explanation** Clearly, when we removed the outlier of size +100, the averageSize of males droped considerably. 
